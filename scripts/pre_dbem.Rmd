...
for(y in 1:length(diat)){
  ...
  name <- paste0("~/Library/CloudStorage/OneDrive-UBC/Data/Climate/cimip6/COBALT2/to_dbem/",forcing,"/totalphy2",str_sub(diat[y],98,101),".txt")
...
for(i in 1:length(variable_list)){
  for(y in 1:length(years)){
    ...
    name <- paste0("~/Library/CloudStorage/OneDrive-UBC/Data/Climate/cimip6/COBALT2/to_dbem/past/",var,yr-10,".txt")
...
test_files <- list.files("~/Library/CloudStorage/OneDrive-UBC/Data/Climate/cimip6/COBALT2/to_dbem/test/",full.names = T)

test_df <- bind_cols(
  lapply(X = test_files,FUN = read.csv, header = F)
) 
colnames(test_df) <- seq(1951,2010,1)

test_df %>% 
  bind_cols(dbem_coords) %>% 
  gather("year","value",`1951`:`2010`) %>% 
  filter(value > -100) %>% 
  group_by(year) %>% 
